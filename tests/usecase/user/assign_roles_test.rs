...
impl UserRepository for MockUserRepo {
    async fn find_by_id(&self, _id: &str) -> Result<domain::User, domain::Error> {
        self.user.lock().unwrap().clone().ok_or(domain::Error::NotFound)
    }
...
async fn test_assign_roles_success() {
    ...
    let uc = AssignRolesUseCaseImpl::new(
        Arc::new(MockUserRepo { user: Arc::new(std::sync::Mutex::new(Some(user))) }),
        Arc::new(MockRoleRepo { role_id: role_id.clone() }),
    );

    let result = uc.execute("1", AssignRolesInput { role_ids: vec![role_id] }).await;
    assert!(result.is_ok());
}
