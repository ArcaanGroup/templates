...
use go_clean_template::application::usecase::user::{GetUserInput, GetUserUseCase, GetUserUseCaseImpl, UserRepository};
...
impl UserRepository for MockUserRepo {
    async fn find_by_id(&self, _id: &str) -> Result<domain::User, domain::Error> {
        self.result.lock().unwrap().clone().unwrap_or(Err(domain::Error::NotFound))
    }
...
async fn test_get_user_success() {
    ...
    let repo = MockUserRepo { result: Arc::new(std::sync::Mutex::new(Some(Ok(user)))) };
...
async fn test_get_user_not_found() {
    let repo = MockUserRepo { result: Arc::new(std::sync::Mutex::new(Some(Err(domain::Error::NotFound)))) };
