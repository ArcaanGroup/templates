...
def validate_config(config: Config) -> List[str]:
    ...
    if not os.path.exists(".env"):
        errors.append("Environment file (.env) not found. Please create it based on .env.example")
...
def ensure_valid_config() -> Config:
    ...
    try:
        ...
        if not os.path.exists(".env"):
            print("Configuration error: Required environment variables are not set.", file=sys.stderr)
            print("\nPlease create a .env file based on .env.example or set the environment variables directly.", file=sys.stderr)
            print("Required variables: DB_USER, DB_PASSWORD, DB_HOST, DB_PORT, DB_NAME", file=sys.stderr)
        else:
            print("Configuration error: Some required environment variables are missing or invalid.", file=sys.stderr)
